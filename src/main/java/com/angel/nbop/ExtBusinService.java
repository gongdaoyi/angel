...
//import com.gf.nbop.busin.counter.model.*;
//import com.gf.nbop.busin.counter.model.form.*;
...
//import com.gf.nbop.busin.counter.service.comm.embedded.custasyn.AsynExecutor;
//import com.gf.nbop.busin.counter.service.comm.embedded.custasyn.AsynExecutorResult;
//import com.gf.nbop.busin.counter.service.comm.embedded.custasyn.ExecutorStatus;
...
//import com.gf.nbop.plat.commonutils.models.constant.*;
//import com.gf.nbop.plat.commonutils.models.mybatis.*;
...
//import com.gf.nbop.plat.commonutils.tools.*;
...
//
//	//BOPS-18474: 新三板/北交所权限开通校验改异步，102正常运行后删除
//	@Value("${nbop.async.sprint102:true}")
...
//	 * */
//	public CommonResponseForm<DataPageOutput<Map<String, Object>>> getBusinAcptformFuzzy(String clientId, String fullName, String idKind, String idNo, String businOpType,
//			Integer startPage, Integer pageSize, String order) throws BopsException {
//
//		CommonResponseForm<DataPageOutput<Map<String, Object>>> extCommonResponseForm = new CommonResponseForm<>();
//		//客户号为空时那就根据三要素查询
...
//			}
//		}
//		// BOPS-21811:重构作废BusinAcptformDao.getBusinAcptformFuzzy(),改用getFuzzy()
//		DataPageOutput<Map<String, Object>> data = businAcptSrv.getFuzzy(clientId, null/* fundAccount */, fullName, idKind, idNo, null/* enBranchNo */, businOpType/* enBusinOpType */, null/* enAcptStatus */,
//				null/* joinBusinessId */, startPage, pageSize, order);
...
//
//		// BOPS-21811:重构作废BusinAcptformDao.getBusinAcptformFuzzy(),改用getFuzzyData()
//		List<JSONObject> datas = businAcptSrv.getFuzzyData(clientId, null/* fundAccount */, fullName, idKind, idNo, null/* enBranchNo */, businOpType/* enBusinOpType */,
//				null/* enAcptStatus */, null/* joinBusinessId */, 1/* startPage */, 1/* pageSize */, "position_str desc"/* order */);
//		if (datas == null || datas.size() == 0) {
//			logger.info2("{},{},{},{}无{}业务", clientId, fullName, idKind, idNo, businOpType);
//			return new RestResult<>("无办理中业务");
//		} else {
//			logger.info2("{},{},{},{}上一笔{}业务：{}", clientId, fullName, idKind, idNo, businOpType, datas.get(0));
//			String originStatus = "," + StringUtils.NVL(datas.get(0).get("acpt_status"), "").toString() + ",";
...
//			, String fullName, String organFlag, String idKind, String idNo, String remark, String businOpType, String terminalType, String joinBusinessId, Integer corpRiskLevel, String acodeAccount){
//
...
//			// BOPS-21811:重构作废BusinAcceptService.getBusinAcptformFuzzy(),改用getFuzzyData()
//			List<JSONObject> runningDatas = businAcptSrv.getFuzzyData((String) businMap.get("clientId"), (String) businMap.get("fundAccount"), (String) businMap.get("fullName"),
//					(String) businMap.get("idKind"), (String) businMap.get("idNo"), null/* enBranchNo */, businOpType/* enBusinOpType */, "6"/* 6-处理中（即跑批未结束） *//* enAcptStatus */,
//					null/* joinBusinessId */, 1/* startPage */, 1/* pageSize */, "position_str"/* order */);
//			if (runningDatas != null && runningDatas.size() != 0) {
//				throw new BopsException(GFErrorCode.ERR_BOPS_SYSTEM_BUSY, GFErrorCode.ERR_BOPS_SYSTEM_BUSY_INFO, "未完成单号：" + runningDatas.get(0).get("acpt_id"));
//			}
//		}
//		// BOPS-12318:支持按功能配置不同兜底检查
//		if (batchTaskCheckServiceIds.contains(businOpType)) {
...
//				boolean useInputClientInfo = StringUtils.isBlank(clientId);// BOPS-20804:非现场关联关系确认允许非我司客户办理，无客户号：让电签模块直接使用上送三要素
//
...
//						idKind,  idNo, opRemark, organFlag, financeType, opEntrustWay, actionInEsign, configBusinType, useInputClientInfo);
...
//								int csdcTransCount = csdcStbTransAMapper.count(0/* initDate */, null/* stockMaxDate */, null/* branchNos */, ""/* csdcTransOutAcct */,
//										holderJson.getString("stock_account")/* csdcTransInAcct */, null/* stockCodes */, null/* enCsdcTransType */, null/* csdcAcodeAccount */,
//										null/* clientId */, null/* isCheckBranch */);
//								if(csdcTransCount > 0) {
//									//当前股东卡在股转过入账户中，进行销户
//									strStatus[197] = '1';
//									String tcheckStr = Arrays.toString(strStatus).replaceAll("[\\[\\]\\s,]", "");
...
//			// BOPS-21811:重构作废BusinAcptformDao.getBusinAcptformFuzzy(),改用getFuzzyData()
//			List<JSONObject> data = businAcptSrv.getFuzzyData(clientId, null/* fundAccount */, fullName, idKind, idNo, String.valueOf(branchNo)/* enBranchNo */,
//					businOpType/* enBusinOpType */, null/* enAcptStatus */, null/* joinBusinessId */, 1/* startPage */, 1/* pageSize */, "position_str desc"/* order */);
//			if (!checkExists || data == null || data.size() == 0) {// BOPS-18646:(移动双录)无跑批无处理，工单状态一直为6，需支持提交新单
//				if (businOpType.equals("900900")) {
//					Map<String, Object> businacptform = acptDao.qryBusinAcptformByJoinId(joinBusinessId);
//					if(businacptform != null){
...
//				String originStatus = "," + StringUtils.NVL(dataMap.get("acpt_status"), "").toString() + ",";
...
//					acptDao.insertBusinAcptformArgExt(dataMap.get("acpt_id").toString(), request.getBusinAcptformArgs());
//					map.put("acptId", (dataMap.get("acpt_id").toString()));
//				} else {
//					throw new BopsException(GFErrorCode.ERR_NBOP_EXTCENTER_ACPTTASK_EXISTS, "存在办理中的受理单，请勿重复提交业务数据！");
//				}
//			}
...
//		// BOPS-20727 GongDaoYi 易涛金未登录办理时，无客户编号，但有营业部优化
//		if (StringUtils.isEmpty(clientId)) {
//			if (branchNo == null) {
//				throw new BopsException("-1", "客户编号和营业部号不能同时为空");
//			}
//			if (branchNo == 0) {
//				throw new BopsException("-1", "客户编号为空时，营业部号不允许为0");
//			}
//		}
//
...
//		if (asyncFlag) {
//			return checkSTNClientRegExtSync(clientId);
//		} else {
//			//BOPS-18474: 新三板权限开通校验改异步，102正常运行后删除
//			return checkSTNclientRegExtOld(clientId);
//		}
//	}
//
//	/**
...
//
//		// 查询适当性信息
//		Map<String, Object> appropriateInfo_0008 = appropriateService.qryAppropriateInfo(/*appropriateType*/"0008", clientId);
//		Map<String, Object> appropriateInfo_0037 = appropriateService.qryAppropriateInfo(/*appropriateType*/"0037", clientId);
//		Map<String, Object> appropriateInfo_0038 = appropriateService.qryAppropriateInfo(/*appropriateType*/"0038", clientId);
//
//		// 策略集合
//		List<Integer> enAppropriateNos_0008 = (List<Integer>) appropriateInfo_0008.get("en_appropriate_no");
//		List<Integer> enAppropriateNos_0037 = (List<Integer>) appropriateInfo_0037.get("en_appropriate_no");
//		List<Integer> enAppropriateNos_0038 = (List<Integer>) appropriateInfo_0038.get("en_appropriate_no");
//
//		// 返回信息1: 允许开立的股东卡
//		ArrayList enAccounts = (ArrayList) appropriateInfo_0008.get("en_account");
//		// 返回信息2: 客户信息
//		Map client = (Map) appropriateInfo_0008.get("client");
...
//		// 新三板合格投资者适合的策略集合列表
//		JSONObject appropriateLgoBase = new JSONObject();
//		for (int i = 0; i < enAppropriateNos_0008.size(); i++) {
//			contextMap.put("appropriateNo", Integer.valueOf(enAppropriateNos_0008.get(i).toString()));
//			appropriateLgoBase = appropriateService.qryAppropriatealgo(contextMap, Integer.valueOf(enAppropriateNos_0008.get(i).toString()), clientId);
//		}
//
//		ArrayList<JSONObject> appropriateLgoExists = (ArrayList<JSONObject>) (appropriateLgoBase.get("data"));
//
//		AsynExecutor executor = new AsynExecutor();
//		JSONObject syncIndexMsg = new JSONObject();
//
//		// BOPS-9612:20200304,hanjin,add,增加股东卡的传送，以便正确识别客户是否是受限投资者
//		for (int i = 0; i < enAccounts.size(); i++) {
//			Map<String, Object> enAccount = (Map<String, Object>) enAccounts.get(i);
//			String stockAccount = enAccount.get("stock_account").toString();
//
//			List<Integer> index0008List = new ArrayList<>();
//			for (int k = 0; k < enAppropriateNos_0008.size(); k++) {
//				Integer appropriateNo = Integer.valueOf(enAppropriateNos_0008.get(k).toString());
//				int index = executor.submitIndex(() ->
//						this.qryAppropriateLgoSync(appropriateNo, contextMap, appropriateLgoExists, clientId, stockAccount));
//				index0008List.add(index);
//			}
//
//			List<Integer> index0037List = new ArrayList<>();
//			for (int k = 0; k < enAppropriateNos_0037.size(); k++) {
//				Integer appropriateNo = Integer.valueOf(enAppropriateNos_0037.get(k).toString());
//				int index = executor.submitIndex(() ->
//						this.qryAppropriateLgoSync(appropriateNo, contextMap, appropriateLgoExists, clientId, stockAccount));
//				index0037List.add(index);
//			}
//
//			List<Integer> index0038List = new ArrayList<>();
//			for (int k = 0; k < enAppropriateNos_0038.size(); k++) {
//				Integer appropriateNo = Integer.valueOf(enAppropriateNos_0038.get(k).toString());
//				int index = executor.submitIndex(() ->
//						this.qryAppropriateLgoSync(appropriateNo, contextMap, appropriateLgoExists, clientId, stockAccount));
//				index0038List.add(index);
//			}
//
//			JSONObject typeAndIndex = new JSONObject();
//			typeAndIndex.put("0008", index0008List);
//			typeAndIndex.put("0037", index0037List);
//			typeAndIndex.put("0038", index0038List);
//
//			syncIndexMsg.put(stockAccount, typeAndIndex);
//		}
//
//		if (executor.getExecutorFinalStatus() == ExecutorStatus.FAIL) {
//			throw new BopsException("-1", "调用失败: ", executor.getAllFailResultMap().toString());
//		}
...
//			Map<String, Object> enAccount = (Map<String, Object>) enAccounts.get(i);
//			String stockAccount = enAccount.get("stock_account").toString();
//			StringBuilder enSstReportType = new StringBuilder();
//
//			JSONObject appropriateLgo = new JSONObject();
//
//			JSONObject typeAndIndex = syncIndexMsg.getJSONObject(stockAccount);
//			for (String key : typeAndIndex.keySet()) {
//				if ("0008".equals(key)) {
//					List indexList = typeAndIndex.getObject("0008", List.class);
//					JSONObject classObj = this.handleSyncResult(indexList, syncResult, enSstReportType, /*sstReportType*/"3", clientId, stockAccount, /*appropriateType*/"0008");
//					appropriateLgo.put("class1", classObj);
//					continue;
//				}
//				if ("0037".equals(key)) {
//					List indexList = typeAndIndex.getObject("0037", List.class);
//					JSONObject classObj = this.handleSyncResult(indexList, syncResult, enSstReportType, /*sstReportType*/"4", clientId, stockAccount, /*appropriateType*/"0037");
//					appropriateLgo.put("class2", classObj);
//					continue;
//				}
//				if ("0038".equals(key)) {
//					List indexList = typeAndIndex.getObject("0038", List.class);
//					JSONObject classObj = this.handleSyncResult(indexList, syncResult, enSstReportType, /*sstReportType*/"5", clientId, stockAccount, /*appropriateType*/"0038");
//					appropriateLgo.put("class3", classObj);
//				}
//			}
//
//			enAccount.put("enSstReportType", enSstReportType);
//			enAccount.put("appropriatealgos", appropriateLgo);
//		}
//
//		JSONObject resultJson = new JSONObject();
//		resultJson.put("errorNo", "0");
//		resultJson.put("errorInfo", "");
//		resultJson.put("enAccounts", enAccounts);
//		resultJson.put("client", client);
//		resultJson.put("remark", "周边新三板权限开通校验接口调用成功");
//
//		//BOPS-10103:20200507,hanjin,add,增加新三板权限开通校验中资产统计日期
//		String busiDate = this.getBusiDate(resultJson, clientId);
//		resultJson.put("busiDate", busiDate);
//
//		// BOPS-10103:20200507,hanjin,add,返回当前自然日最近交易日T-1日日期
//		String nextTradeDate = this.getNextTradeDate(resultJson);
//		resultJson.put("preTradeDate", nextTradeDate);
//
...
//
//	/**
//	 * 查询客户选择开立对应合格投资者类型的策略集合信息
//	 */
//	private JSONObject qryAppropriateLgoSync(Integer appropriateNo, Map<String, Object> contextMap, ArrayList<JSONObject> appropriateLgoExists, String clientId, String stockAccount) throws BopsException {
//		Map<String, Object> syncMap = new HashMap<>(contextMap);
//		syncMap.put("stockAccount", stockAccount);
//
//		JSONObject appropriateLgo = appropriateService.qryAppropriatealgo2(syncMap, appropriateNo, appropriateLgoExists, clientId);
//
//		JSONObject result = new JSONObject();
//		Boolean totalLogicValue = appropriateLgo.getBoolean("total_logic_value");
//
//		result.put("appropriateNo", appropriateNo);
//		result.put("class", appropriateLgo);
//		result.put("totalLogicValue", totalLogicValue);
//		result.put("errorNo", "0");
//
//		return result;
//	}
//
//	private JSONObject handleSyncResult(List indexList, ArrayList<AsynExecutorResult> syncResult, StringBuilder enSstReportType,
//										String sstReportType, String clientId, String stockAccount, String appropriateType) throws BopsException {
//		String errorInfo = clientId + ":" + stockAccount + ":" + appropriateType;
//		JSONObject classSync = null;
//
//		for (Object obj : indexList) {
//			Integer index = (Integer) obj;
//			AsynExecutorResult<JSONObject> subResult = syncResult.get(index);
//
//			if (subResult.getStatus() == ExecutorStatus.SUCESS) {
//				JSONObject resData = subResult.getResult();
//				String appropriateNo = resData.getString("appropriateNo");
//
//				if (!ObjectUtils.isEmpty(resData) && "0".equals(resData.getString("errorNo"))) {
//					classSync = resData.getJSONObject("class");
//
//					Boolean totalLogicValueSync = resData.getBoolean("totalLogicValue");
//					if (totalLogicValueSync) {
//						enSstReportType.append(enSstReportType.length() == 0 ? sstReportType : "," + sstReportType);
//					}
//				} else {
//					logger.error("异步第[{}]笔[{}]执行成功，结果异常: {}", index, errorInfo + ":" + appropriateNo, resData);
//					throw new BopsException(GFErrorCode.ERR_BOPS_INNER_EXCEPTION, GFErrorCode.ERR_BOPS_INNER_EXCEPTION_INFO);
//				}
//			} else if (subResult.getStatus() == ExecutorStatus.FAIL) {
//				logger.error("异步第[{}]笔[{}]执行失败", index, errorInfo);
//				BopsException e = (BopsException) subResult.getException();
//				throw new BopsException(e.getErrCode(), e.getErrMessage());
//			} else {
//				logger.error("异步第[{}]笔[{}]执行异常，状态为: {}", index, errorInfo, subResult.getStatus().getValue());
//				throw new BopsException(GFErrorCode.ERR_BOPS_INNER_EXCEPTION, GFErrorCode.ERR_BOPS_INNER_EXCEPTION_INFO);
//			}
//		}
//
...
//	 * 获取资产统计日期
//	 */
//	private String getBusiDate(JSONObject resultJson, String clientId) {
//		//BOPS-10103:20200507,hanjin,add,增加新三板权限开通校验中资产统计日期
...
//	 * 获取当前自然日最近的T-1交易日日期
//	 */
//	private String getNextTradeDate(JSONObject resultJson) throws BopsException {
//		// BOPS-10103:20200507,hanjin,add,返回当前自然日最近交易日T-1日日期：
//		// 比如自然日为周五，返回周四日期；
//		// 自然日为周日，返回周五日期；
//		// 自然日为周一，返回上周五日期。（假设周一至周五均为交易日）
//		Integer currDate = StringUtils.getYYYYMMDD(new Date());
//		JSONObject dateObj = outerInterfaceService.getNextTradeDate("1", "-1", currDate.toString());
//
//		//返回当前自然日最近的T-1交易日日期
//		return dateObj.getString("next_trade_date");
//	}
//
//
//	/**
...
//	 * @Link   BOPS-9461*
//	 * */
//	public CommonResponseForm<JSONObject> checkSTNclientRegExtOld(String clientId) throws BopsException {
//		// BOPS-18533 高龄客户校验
//		this.checkAge(clientId);
//
//		JSONObject retLast = new JSONObject();
...
//		if (asyncFlag) {
//			return checkBJSclientRegExtSync(clientId);
//		} else {
//			//BOPS-18474: 北交所权限开通校验改异步，102正常运行后删除
//			return checkBJSclientRegExtOld(clientId);
//		}
//	}
//
//	public String oneStopOpenSTNAndBJSExt(String clientId, Integer branchNo, String fundAccount, String TZAaccount, String enFundAccount,
//										  String enExchangeKind, String enStockAccount, String enSstReportType) throws BopsException {
//
//		return "";
//	}
//
...
//	 * @jira: BOPS-18474
//	 */
//	public CommonResponseForm<JSONObject> checkBJSclientRegExtSync(String clientId) throws BopsException {
//		// BOPS-18533 高龄客户校验
//		this.checkAge(clientId);
//
//		// 查询适当性信息
//		Map<String, Object> appropriateInfo_0053 = appropriateService.qryAppropriateInfo(/*appropriateType*/"0053", clientId);
//
//		// 策略集合
//		List<Integer> enAppropriateNos_0053 = (List<Integer>) appropriateInfo_0053.get("en_appropriate_no");
//
//		// 返回信息1: 允许开立的股东卡
//		ArrayList enAccounts = (ArrayList) appropriateInfo_0053.get("en_account");
//		// 返回信息2: 客户信息
//		Map client = (Map) appropriateInfo_0053.get("client");
//		if (CollectionUtils.isEmpty(enAccounts)) {
//			throw new BopsException(GFErrorCode.ERR_NBOP_BUSIN_NO_SATISFY_ACCOUNT, GFErrorCode.ERR_NBOP_BUSIN_NO_SATISFY_ACCOUNT_INFO);
//		}
//
...
//		// 新三板合格投资者适合的策略集合列表
//		JSONObject appropriateLgoBase = new JSONObject();
//		for (int i = 0; i < enAppropriateNos_0053.size(); i++) {
//			contextMap.put("appropriateNo", Integer.valueOf(enAppropriateNos_0053.get(i).toString()));
//			appropriateLgoBase = appropriateService.qryAppropriatealgo(contextMap, Integer.valueOf(enAppropriateNos_0053.get(i).toString()), clientId);
//		}
//
//		ArrayList<JSONObject> appropriateLgoExists = (ArrayList<JSONObject>) (appropriateLgoBase.get("data"));
//
...
//			String stockAccount = enAccount.get("stock_account").toString();
//
//			List<Integer> index0053List = new ArrayList<>();
//			for (int k = 0; k < enAppropriateNos_0053.size(); k++) {
//				Integer appropriateNo = Integer.valueOf(enAppropriateNos_0053.get(k).toString());
//				int index = executor.submitIndex(() ->
//						this.qryAppropriateLgoSync(appropriateNo, contextMap, appropriateLgoExists, clientId, stockAccount));
//				index0053List.add(index);
//			}
//
//			JSONObject typeAndIndex = new JSONObject();
//			typeAndIndex.put("0053", index0053List);
//
//			syncIndexMsg.put(stockAccount, typeAndIndex);
//		}
//
...
//			for (String key : typeAndIndex.keySet()) {
//				if ("0053".equals(key)) {
//					List indexList = typeAndIndex.getObject("0053", List.class);
//					String bjsReportType = memCache.getSysconfigValue(8888, "1000210");
//					JSONObject classObj = this.handleSyncResult(indexList, syncResult, enSstReportType, bjsReportType, clientId, stockAccount, /*appropriateType*/"0053");
//					appropriateLgo.put("class1", classObj);
//				}
//			}
//
...
//		resultJson.put("enAccounts", enAccounts);
//		resultJson.put("client", client);
//		resultJson.put("remark", "非现场北交所股票交易权限开通校验成功");
//
//		//BOPS-10103:20200507,hanjin,add,增加北交所股票交易权限开通校验中资产统计日期
//		String busiDate = this.getBusiDate(resultJson, clientId);
//		resultJson.put("busiDate", busiDate);
//
//		// BOPS-10103:20200507,hanjin,add,返回当前自然日最近交易日T-1日日期
...
//		resultFrom.setErrorNo("0");
//		resultFrom.setErrorInfo("");
//		resultFrom.setRemark("非现场北交所股票交易权限开通校验成功");
//		return resultFrom;
//	}
//
//
...
//	 * DaoYiGonG
//	 */
//	public CommonResponseForm<JSONObject> checkBJSclientRegExtOld(String clientId) throws BopsException {
//		// BOPS-18533 高龄客户校验
//		this.checkAge(clientId);
//
//		JSONObject retLast = new JSONObject();
...
//		}
//
//		//BOPS-22678:hanjin,add,增加产品户的organprodinfo返回
//		if (Constant.ORGAN_FLAG_ORGANPROD.equals(clientBean.getOrganFlag()) || Constant.ORGAN_FLAG_PUBFUND.equals(clientBean.getOrganFlag())) {
//			Organprodinfo organprodinfoBean = JSON.toJavaObject(totalInfo.getJSONObject("organprodinfo"), Organprodinfo.class);
//			output.setOrganprodinfo(organprodinfoBean);
//		}
//
...
//	}
//
//	public CommonResponseForm<JSONObject> qryBankAccountsByCond(String clientId, String fundAccount, String bankNo, String bankType, String moneyType, String actionStr) throws BopsException {
//
//		CommonResponseForm<JSONObject> extCommonResponseForm = new CommonResponseForm<JSONObject>();
//    	JSONObject retLast = outerInterfaceService.qryBankAccountsByCond(clientId, fundAccount, bankNo, moneyType, bankType, actionStr);
//
//		extCommonResponseForm.setData(retLast);
//		extCommonResponseForm.setErrorNo("0");
//		extCommonResponseForm.setErrorInfo("");

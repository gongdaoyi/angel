...
public class SqlExecuteTimeCountInterceptor implements Interceptor {
    ...
    private String limitSQLLength(String sql, Object parameterObject, List<ParameterMapping> parameterMappingList) {
        ...
        for (ParameterMapping parameterMapping : parameterMappingList) {
            ...
            String detail = "[" + propertyName + ":" + value + "]；";
            paramsBuilder.append(detail);

            // 替换SQL中的 #{...} 或 ? 为实际参数值
            if (value != null) {
                String paramValue = value instanceof String ? "'" + value + "'" : String.valueOf(value);
                finalSql = finalSql.replaceFirst("\\?", paramValue);
            }
        }

        finalSql += paramsBuilder.toString();
        if (finalSql.length() > MAX_SQL_LENGTH) {
            return finalSql.substring(0, MAX_SQL_LENGTH);
        } else {
            return finalSql;
        }
    }
